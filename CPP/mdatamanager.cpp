...
void MDataManager::loadFile(QString __fileName)
{
    ...
    case PIC:
    {
        ...
        for(i=0;i<m_mng->GetNumOperativeMarkers();i++)
        {
            ...
            {//marker conosciuto le info ce le ho gi√
                (*mrk)=m_markerMap[key];
            }
            else
            {//me lo costruisco
...
bool MDataManager::buildInfoList()
{
    ...
    //allora dato che questa funzione √® chiamata dopo aver costruito i plotter
    //so gi√  quanti e come si chiamano i grafici
...
bool MDataManager::saveDataAndUpdate(QString __family, QString __name, VarMapVec* __elements, bool __whatIfAlreadyPresent)
{
    ...
    {//se siamo qua dentro vuol dire che tutto √® andato liscio e possiamo visualizzare le info all'utente
...
void MDataManager::exitFromReview()
{
    ...
    else
    {
        ...
        {        //copio il file copy nell'originale
            ...
            qDebug()<<"cancellata copia all'exit"<<QFile::remove(copyName);
        g_mainAppBridge->sendSwitch(); //poi dovr‡ tornare al modulo database
    }
...
bool MDataManager::checkForVolRes()
{
    ...
    //ciclo per individuare se √® necessario aprire la dlg del volume residuo
    bool volRes = false;
...
qDebug()<<"INIZIO";
    ...
    for (int i=0; i<m_numAna; i++)
    {
        ...
        if (anaType == FLW_AVD_STUDY)
        {
            //verifica se c'√® un definitore per questa analisi.
            VarMap mkOpAnIn;
            bool found = false;
            ...
            if (!found)
            {
                ...
                {
                    //se non c'√® il defintore, ma questa √® l'unica analisi,
                    ...
                    //ho tralasciato la parte che gestisce l'iconizzazione che forse non c'Ë
                ...
                //                else
                //                {
                //                    //cotrolliamo se c'√® il marker 'ff' free flow
                //                    //nel qual caso, il definitore viene inserito da l√¨ fino a:
                ...
                //                                //ho tralasciato la parte che gestisce l'iconizzazione che forse non c'√®
            ...
            if (found)
            {
                ...
                //                                            'su DlgReport il messaggio d'errore anzich√® il risultato.
    ...
    if (__anaType == FLW_AVD_STUDY)
    {
        ...
        foreach (VarMap *curMap, (*elements))
        {
            ...
            if (key == MK_FLOWMETRY)
            {
                ...
                //√® necessario sistemare l'array dei canali abilitati considerando tutti i canali dell'analisi.
