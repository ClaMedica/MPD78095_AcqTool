...
Rectangle {
    ...
    function start(__directConnectionToHost) { plotter.start(__directConnectionToHost); running=true; console.log(modP.serverPort,"plot start")}
    ...
    onModelloChanged:
    {
        ...
        //console.log("changed",modello.length)
        legend.model = modP.tName
    }
...
//        }
    ...
    Rectangle {
        ...
        property real vminparent: plotter.yMin
        property real vmaxnparent: plotter.yMax
        ...
        Plot2DRealTime {
            ...
            onLastSampleChanged:
            {
                if(modP.yAutoScale)
                {
                    while (lastSample[1] > modP.yAbsoluteMax)
                    {
                        var valRange = mod.range.split("#")
                        var numero = valRange[2]*1
                        var newLimit = mod.yAbsoluteMax + numero
                        var max = valRange[0]*1
                        if (newLimit <= max)
                            modP.yAbsoluteMax = newLimit
                        else
                            break;
                    }

                    while (lastSample[1] < modP.yAbsoluteMin)
                    {
                        var valRangeMin = mod.range.split("#")
                        var num = valRangeMin[2]*1
                        var newLimitMin = mod.yAbsoluteMin - num
                        var min = -valRangeMin[0]*1
                        if (newLimitMin >= min)
                            modP.yAbsoluteMin = newLimitMin
                        else
                            break;
                    }
                }
                horMark.value=lastSample[1]
            }
        }
        ...
        MCursorY
        {
            ...
            vMax:rCenter.vmaxnparent//plotter.yMax
            vMin:rCenter.vminparent//plotter.yMin
            colore:"black"
